...
def test_delete_user():
    ...
    with db.connect() as connection:
        ...
        user = result.first()
        assert user is not None, "Не удалось создать тестового пользователя"
...
def test_update_user():
    ...
    with db.connect() as connection:
        ...
        connection.execute(
            text(
                "UPDATE users SET user_email = :new_email, subject_id = :new_subject_id WHERE user_email = :old_email"),
            {"new_email": new_email, "new_subject_id": new_subject_id, "old_email": test_email}
        )
        connection.commit()
